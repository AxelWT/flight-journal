...
```bash
# 从已有分支创建 worktree
# 这行命令会在当前仓库同级目录下创建一个新目录 hotfix-dir，里面是 hotfix-branch 分支的一份独立工作副本。
# 直接用 hotfix-branch 分支的问题： 你在 hotfix-dir 目录下做的任何提交都会直接提交到 hotfix-branch 分支，绕开了正常的开发流程。
git worktree add ../hotfix-dir hotfix-branch

# 创建新分支并关联到 worktree
# 这是更推荐的使用方式！切新的 worktree 时最好检出一个新的分支
